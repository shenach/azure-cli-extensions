...
@register_command(
    "providerhub sku create",
)
class Create(AAZCommand):
    """Create the resource type skus in the given resource type.

    :example: Skus_CreateOrUpdate
        az providerhub sku create --provider-namespace "{providerNamespace}" --resource-type "{resourceType}" --name "{skuName}" --sku-settings "[{name:freeSku,tier:Tier1,kind:Standard},{name:premiumSku,tier:Tier2,kind:Premium,costs:[{meter-id:xxx}]}]"
    """
